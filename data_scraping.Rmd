...
get.cov(artist = "Daft Punk", song = "One More Time")
rem <- get.rem(artist = "Daft Punk", song = "One More Time")


rem$artist <- gsub("\n\t\t\t", "", rem$artist, fixed = T)
rem$artist <- sub("by ", "", rem$artist, fixed = T)
rem <- separate(rem, artist, into = c("artist", "year"), "\\ (")
rem$year <- str_sub(rem$artist,-6,-1)

rem$artist <- str_sub(rem$artist, 2, 3)
