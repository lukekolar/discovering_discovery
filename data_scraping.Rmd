...
```{r, include = F}
samp.tibble(artist = "Drake", song = "God's Plan")

get.cov(artist = "Drake", song = "God's Plan")
```
...
discogs_info.tib <- function(artist, song){
  result <- discogs_search(list(track = song, artist = artist), n_results = 1)
  tibble(song = song,
         artist = artist,
         style = result[[1]]$style,
         country = unlist(result[[1]]$country),
...
discogs_info.tib(pull(x[1,2]), pull(x[1,1]))
