...
full_join((lyrics %>% 
  group_by(song_name) %>% 
  summarize(total = n())), by = "song_name")

check_line_repetition <- function(songname){
  
  lyrics %>% filter(song_name == songname) %>% 
    group_by(song_name) %>% 
    distinct(line) %>% 
    summarize(distinct = n()) %>% 
  full_join((lyrics %>% filter(song_name == songname) %>%  
    group_by(song_name) %>% 
    summarize(total = n())), by = "song_name") %>% 
    mutate(number = total/distinct) %>% 
    pull(number)
  
}

check_word_repetition <- function(songname){

  collapsed_string <- paste(as.vector(t((lyrics %>% 
    filter(song_name == songname))$line)), sep = " ", collapse = " ")
  
  totalwords <- sapply(strsplit(collapsed_string, " "), length)
  
  split_string <- tibble(words = unlist(strsplit(collapsed_string, " ")))
  
  uniquewords <- split_string %>% 
    distinct(words) %>% 
    summarize(distinct = n()) %>% 
    pull(distinct)
...
lyrics %>% 
  distinct(song_name) %>% 
  mutate(line_rep = map(song_name, ~check_line_repetition(songname = .)),
         word_rep = map(song_name, ~check_word_repetition(songname = .))) %>% 
  unnest(cols = c(line_rep, word_rep)) %>% 
  left_join(readRDS("discovering_discovery/scraped_data/discovery_song_qualities.RDS"), 
                         by = c("song_name" = "track_name")) %>% 
          select(song_name, line_rep, word_rep, total.uses) %>% 
  saveRDS("discovering_discovery/scraped_data/discovery_repetition_index.RDS")

repetition <- readRDS("discovering_discovery/scraped_data/discovery_repetition_index.RDS")
repetition
...
check_line_repetition(songname = "One More Time")

check_word_repetition(songname = "Digital Love")
...
p2noutlm <- p2out + geom_smooth(method = "lm", formula = y ~ x, se = F)
...
```{r}
create.rep.graph(type = "Word Repetition (remove outlier)", method = "Linear")
...
```{r}
create.outlier.word.rep("Linear")

create.no.outlier.word.rep("Linear")

create.no.outlier.line.rep("Linear")

create.outlier.line.rep("None")
...
# anim_save("discovering_discovery/misc/songs.anim.gif", ani.width= 1000, ani.height=1000)
# include_graphics("discovering_discovery/misc/songs.anim.gif")
...
tracks <- c("One More Time", "Aerodynamic", "Digital Love",
            "Harder, Better,...", "Crescendolls", 
            "Nightvision", "Superheroes", "High Life", 
            "Something About Us", "Voyager", "Veridis Quo",
            "Short Circuit", "Face to Face", "Too Long")
...
plot <- ggplot() + geom_hline(yintercept = 0.5) + xlim(1971, 1983) + ylim(0.5, 0.65) +
  geom_point(data = data.timeline, aes(x = year, y = 0.5), size = 3)
ggplotly(plot)
...
create.type.summary.plot <- function(year.slide){
  ...
  ggplot((data.main %>% 
    ...
    theme(text = element_text(family = "Courier"),
                 axis.text.y = element_text(angle = 60, size = 14, hjust = 0.66, 
                                            vjust = 0, face = "bold"),
                 axis.text.x = element_text(size = 10),
                 axis.ticks.y = element_blank()) +
    labs(x = "", y = "") +
    geom_label(data = label, aes(label = label), fontface = "bold", 
               size = 10, family = "Courier", label.padding = unit(0.5, "lines"), 
               color = "white", fill = "steelblue")
}
...
```{r}
create.alt.uses.per.year.plot("December 2010")
```
...
create.uses.per.year.plot <- function(year.slide){
  ...
  ggplot(data, aes(x = year, y = n, fill = type)) + 
    ...
    scale_y_continuous(expand = c(0,0.15), limits = c(0, 24)) +
    scale_fill_manual(values = c("turquoise2", "steelblue1", "turquoise4"), name = "Credit type",
                      breaks = c("Samples", "Covers", "Remixes")) +
    theme(plot.margin = unit(c(1,1,1,1.5), "cm")) +
    geom_rect(aes(xmin = year.slide - 0.5, xmax = year.slide + 0.5, ymin = 0, 
             ymax = (data %>% filter(year == year.slide) %>% nrow())),
             fill = NA, color = "red", size = 0.4) +
    geom_label(aes(label = paste0((data %>% filter(year == year.slide) %>% nrow()), " total uses"),
                   x = 2004, y = 19), 
...
create.line.and.uses.patch <- function(year.slide){
    ...
    p1 <- create.uses.line.plot(year.slide = year.slide)
    p2 <- create.uses.per.year.plot(year.slide = year.slide)
    
    grid.arrange(p1, p2, ncol = 2)

}

create.line.and.uses.patch(2019)
```
...
credit.chronology <- ggplot((
                              data.main %>% 
                                  ...
                                  mutate(type = str_replace(type, "sampled", "Samples"),
                                         ...
                                         type = str_replace(type, "cover", "Covers")) %>% 
                                  mutate(type = as.factor(type)) %>% 
                                  mutate(type = factor(type, levels = c("Samples", 
                                                                        "Covers", "Remixes"))) %>% 
                                  group_by(track, artist, year, type) %>% 
                                  distinct(track) %>% 
                                  summarize(n = n())
...
daft_punk_time_search <- readRDS("discovering_discovery/scraped_data/daft_punk_time_search.RDS")
peaks.list <- peaks %>% 
  group_by(time) %>% 
  nest() %>% 
  ungroup() %>% 
  cbind(date = c("2005-03-01", "2007-12-01", "2010-12-01", 
                 "2013-05-01", "2014-01-01", "2017-02-01")) %>%  
  mutate(date = ymd(date)) %>% 
  left_join(daft_punk_time_search, by = c("date")) %>% 
  select(data, date, hits) %>% 
  unnest(cols = data) %>% 
  group_by(hits, date) %>% 
  mutate(terms = list(value)) %>% 
  distinct(date, .keep_all = T) %>% 
  ungroup() %>% 
  select(date, hits, terms)
# peaks.list %>% saveRDS("discovering_discovery/scraped_data/peaks_list.RDS")

peak.dates <- c("2005-03-01", "2007-12-01", "2010-12-01", 
                "2013-05-01", "2014-01-01", "2017-02-01")


daft_punk_time_search <- readRDS("discovering_discovery/scraped_data/daft_punk_time_search.RDS")
peaks.list <- readRDS("discovering_discovery/scraped_data/peaks_list.RDS")
...
")
  ...
  p <- ggplot(data = daft_punk_time_search, aes(x = date, y = hits))+
    ...
    annotate("label", x = ymd("2006-07-01"), y = 90, label = paste0(peak.selection, ":"), 
             family = "Courier", padding.size = 0.5, label.padding = unit(0.5, "lines"), 
             color = "white", fill = "steelblue", size = 4) + 
    annotate("label", x = ymd("2006-07-01"), y = 70, label = paste0(terms.needed), 
...
create.google.trends.plot("January 2014")
...
daft2 <- get_artist_audio_features('Daft Punk') %>% 
  filter(album_name == "Discovery") %>% 
  select(track_name, track_number,
         energy, mode, mode_name, instrumentalness,
         tempo, duration_ms, key, key_name, 
...
daft1 <- data.main %>% 
  ...
  full_join(data.main %>% 
    filter(type == "cover") %>% 
    group_by(main.song) %>% 
    summarize(covered = n()), by = "main.song") %>% 
  full_join(data.main %>% 
    filter(type == "remix") %>% 
    group_by(main.song) %>% 
    summarize(remixed = n()), by = "main.song") %>% 
  mutate(covered = replace_na(covered, 0),
         remixed = replace_na(remixed, 0),
         total.uses = sampled + remixed + covered)

daft <- daft2 %>% full_join(daft1, by = c("track_name" = "main.song"))
daft %>% saveRDS("discovering_discovery/scraped_data/discovery_song_qualities.RDS")

daft <- readRDS("discovering_discovery/scraped_data/discovery_song_qualities.RDS")

corrplot(cor(daft %>% 
               select(energy, mode, instrumentalness,
                      tempo, duration_ms, key, 
                      speechiness, liveness, valence,
                      danceability, loudness, acousticness, 
                      remixed, covered, sampled, total.uses)), 
         type = "upper", insig = "label_sig")

daft %>% ggplot(aes(x = track_name, y = percent_rank(total.uses))) + geom_col() + 
  geom_col(alpha = 0.2, aes(y = percent_rank(valence)))



create.song.comparison.analytics <- function(input1, input2){

  input1 <- "One More Time"
  input2 <- "Nightvision"

  tracks <- c(input1, input2)
  filt.daft <- daft %>% filter(track_name %in% tracks)
...
ggplot(filt.daft, aes(x = track_name, y = key)) + geom_col(aes(fill = track_name)) + 
  geom_hline(yintercept = c(0, 11))

create.song.comparison.analytics(input1 = "One More Time", input2 = "Nightvision")

daft %>% select(track_name, acousticness)
```
...
```{r}
create.reg.plot(variable = "Energy", type = "loess")
