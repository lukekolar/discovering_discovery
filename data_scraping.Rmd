...
sampd.network.raw %>% saveRDS(file = "sampd_network_raw.RDS")
...
sampd.network.raw <- readRDS("sampd_network_raw.RDS")
View(sampd.network.raw)
```
...
pull(data.main %>% distinct(main.song))
...
total_ranks <- num.by.year.tidy %>% 
  filter(type == "remix") %>% 
  select(total.all, main.song, year) %>% 
  group_by(year) %>% 
  mutate(rank = rank(-total.all),
         Value_lbl = paste0(" ", total.all)) %>% 
  select(!total.all)

num.by.year.formatted <- num.by.year.tidy %>% 
  left_join(total_ranks, by = c("main.song", "year"))

?rank

View(num.by.year.formatted)
