...
library(geniusr)
genius_token()
genius.token <- "A629GZJinyQQJN11vDjRf_3tjx6Dsw5fpH1hdiLZwhpgABHw4yyTpm6nFQj592p8"

# 'Discogs'-specific packages
library(discogger)
discogs_api_token()
discogs.token <- "UqigHPcLrZSSnZuebGkYFAMdYWBWRzvtfHsgtkZK"
```
...
get.cov <- function(artist, song){
  ...
  if(pull(samp.tib[3, 2]) == 0){
    ...
    if(pull(samp.tib[3, 2]) <= 5){
      n1 <- pull(samp.tib[1, 2]) + pull(samp.tib[2, 2]) + 1
...
get.rem <- function(artist, song){
  ...
  if(pull(samp.tib[4, 2]) == 0){
    ...
    if(pull(samp.tib[4, 2]) <= 5){
      n1 <- pull(samp.tib[1, 2]) + pull(samp.tib[2, 2]) + pull(samp.tib[3, 2]) + 1
...
get.samps(artist, song)
s2 <- get.sampd(artist, song) %>% mutate(type = "sampled")
s3 <- get.cov(artist, song) %>% mutate(type = "cover")
s4 <- get.rem(artist, song) %>% mutate(type = "remix")

s1 <- get.samps(artist, song) %>% mutate(type = "samples")
s2 <- get.sampd(artist, song) %>% mutate(type = "sampled")
s3 <- get.cov(artist, song) %>% mutate(type = "cover")
s4 <- get.rem(artist, song) %>% mutate(type = "remix")
...
```{r}
result <- discogs_search(list(track = track, artist = artist), n_results = 1)

result

tib <- get.samps(artist, track)
nrow(tib)

style <- vector("character", nrow(tib))
country <- vector("character", nrow(tib))
genre <- vector("character", nrow(tib))
...
for(k in 1:nrow(tib)){
  ...
  artist <- tib[k, "artist"] 
  result <- discogs_search(list(track = track, artist = artist), n_results = 1)
  style[k] <- result[[1]]$style[[1:n]]
  country[k] <- result[[1]]$country[[1:n]]
  genre[k] <- result[[1]]$genre[[1:n]]
}


for(row in 1:nrow(tib)){
  track <- tib[row, "track"]
  artist <- tib[row, "artist"] 
  result <- discogs_search(list(track = track, artist = artist), n_results = 1)
    if(exists(result[[1]]$style[[1]])){
      style = result[[1]]$style[[1:n]]
    }else{
      style = NA
    }
    if(exists(result[[1]]$country[[1]])){
      country = result[[1]]$country[[1:n]]
    }else{
      country = NA
    }
    if(exists(result[[1]]$genre[[1]])){
      genre = result[[1]]$genre[[1:n]]
    }else{
      genre = NA
    }
}
